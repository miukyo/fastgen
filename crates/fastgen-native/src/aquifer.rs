...
use crate::math::{clamp, map, map_clamped};
use crate::block::{BlockStateId, BLOCK_WATER, BLOCK_LAVA};
...
impl<N: DimensionNoises> Aquifer<N> {
    ...
    pub fn new_sized(
        ...
        let water_id = BLOCK_WATER;
        let lava_id = BLOCK_LAVA;
    ...
    #[inline]
    pub const fn skip_sampling_above_y(&self) -> i32 {
    ...
    pub fn compute_substance(
        ...
        for i in 0..12 {
            let dy = self.col_cache.cell_loc_y[i] - world_y;
            dists[i] = self.col_cache.cell_xz_dist_sq[i] + dy * dy;
        }
