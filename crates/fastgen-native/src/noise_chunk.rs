...
impl<N: DimensionNoises> NoiseChunk<N> {
    ...
    /// Reset coordinate anchor to reuse this NoiseChunk's memory buffers without reallocating.
    #[inline]
    pub fn reset(&mut self, chunk_min_block_x: i32, chunk_min_block_z: i32) {
        let cell_width = N::Settings::CELL_WIDTH;
        self.first_cell_x = chunk_min_block_x.div_euclid(cell_width);
        self.first_cell_z = chunk_min_block_z.div_euclid(cell_width);
    }
        ...
        F: FnMut(usize, i32, usize, f64, &[f64], &mut N::ColumnCache),
    {
        ...
        let n_slices = cell_count_xz + 1;
        let mut local_blended = [0.0f64; MAX_SLICE_LEN];
        ...
        for cell_x_idx in 0..cell_count_xz {
            ...
            for cell_z_idx in 0..cell_count_xz {
                ...
                for cell_y_idx in (0..cell_count_y).rev() {
                    let min_cell_world_y = (self.cell_min_y + cell_y_idx as i32) * cell_height;
                    ...
                    // Early-out air culling:
                    // Above sea level (y >= 64), if all 8 cell corners have negative terrain density (channel 0 < 0.0),
                    // trilinear interpolation is guaranteed strictly negative everywhere in this cell.
                    // Because y >= 64, no aquifer fluids can exist, and no solid blocks can form.
                    // The buffer already holds BLOCK_AIR, so all 4x8x4 = 128 blocks can be safely skipped.
                    if min_cell_world_y >= 64
                        && s0[i0_base] < 0.0
                        && s0[i1_base] < 0.0
                        && s1[i0_base] < 0.0
                        && s1[i1_base] < 0.0
                    ...
                    for x_in_cell in 0..cell_width {
                        ...
                        for z_in_cell in 0..cell_width {
                            ...
                            for y_in_cell in (0..cell_height).rev() {
                                ...
                                let world_y = min_cell_world_y + y_in_cell;
